...
function LegSteps({ steps }: { steps: StepDetail[] }) {
  ...
  return (
    <div className="mt-2 mb-1 rounded-lg overflow-hidden border border-gray-100">
      {steps.map((step, i) => {
        const colors = stepModeColor(step.mode)
        return (
          <div
            key={i}
            className="flex items-start gap-2.5 px-3 py-2 text-xs border-b border-gray-50 last:border-b-0 bg-white hover:bg-gray-50 transition-colors"
          >
            <div
              className="flex-shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium"
              style={{ background: colors.bg, color: colors.text, border: `1px solid ${colors.border}` }}
            >
              <span>{modeIcon(step.mode)}</span>
              <span>{modeLabel(step.mode)}</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-gray-700 leading-snug">{step.instruction}</div>
              {step.line && (
                <div className="text-gray-400 mt-0.5 truncate">
                  Line: <span className="font-medium text-gray-600">{step.line}</span>
                </div>
              )}
            </div>
            <div className="flex-shrink-0 text-gray-400 font-medium tabular-nums whitespace-nowrap">
  ...
  return (
    <div className="flex gap-1 mt-2 flex-wrap">
      {alts.map((alt, i) => {
        ...
        // Summarise the option: primary transit line(s)
        const transitSteps = alt.steps?.filter(s =>
          s.mode !== 'WALK' && s.line
        ) ?? []
        const label = transitSteps.length > 0
        ...
        return (
          ...
          >
            <span>{i + 1}</span>
            <span>{label}</span>
            <span className="opacity-70">· {alt.durationMinutes}m</span>
          </button>
        )
      })}
    </div>
...
export function Timeline({ result, selectedOptions, onSelectOption }: Props) {
  const { legs, stops, totalDurationMinutes, arrivalTime } = result

  // Use selected alternatives for display
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Summary strip */}
      <div className="flex gap-2 px-5 py-3 border-b border-gray-100 text-center flex-shrink-0">
        {[
          { val: `${totalDurationMinutes}m`, label: 'Total' },
          { val: `${transitMin}m`,           label: 'Transit' },
          { val: `${walkMin}m`,              label: 'Walking' },
          { val: `${stayMin}m`,              label: 'Layovers' },
        ].map(({ val, label }) => (
          <div key={label} className="flex-1">
            <div className="text-base font-medium leading-tight">{val}</div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <div className="relative">
              ...
              <div key={i}>
                <div className="flex gap-3 relative">
                  <div className="flex flex-col items-center flex-shrink-0">
                    <div
                      className="w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0 z-10"
                  ...
                  <div className="pb-1 flex-1 min-w-0">
                    <div className="text-xs text-gray-400 font-medium">{active.departureTime}</div>
                    <div className="text-sm font-medium mt-0.5 truncate">{active.from}</div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-gray-400 truncate">
                        {modeLabel(active.mode)}{active.line ? ` · ${active.line}` : ''} → {active.to}
                      </span>
                      <span
                        className="text-xs px-1.5 py-0.5 rounded font-medium flex-shrink-0"
                        style={{ background: `${color}15`, color }}
                      >
                        {active.durationMinutes} min
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 mb-3">
                      ...
                      <span className="text-xs text-gray-400">{active.arrivalTime}</span>
                    </div>
                  </div>
                </div>
                ...
                {hasStay && (
                  <div className="flex gap-3 relative">
                    <div className="flex flex-col items-center flex-shrink-0">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0 z-10 bg-gray-50 border border-gray-200">
                        📍
                      </div>
                      <div className="w-px flex-1 min-h-[24px] bg-gray-100" />
                    </div>
                    <div className="pb-3 flex-1 min-w-0">
                      <div className="text-xs text-gray-400 font-medium">{active.arrivalTime}</div>
                      <div className="text-sm font-medium mt-0.5 truncate">{active.to}</div>
                      <div className="mt-1.5 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                        <div className="text-xs text-gray-400 font-medium">⏱ Staying {midStop.stay} min</div>
          ...
          <div className="flex gap-3">
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0 bg-emerald-50 border border-emerald-200">
              🏁
            </div>
            <div className="pb-2 flex-1 min-w-0">
              <div className="text-xs text-gray-400 font-medium">{arrivalTime}</div>
              <div className="text-sm font-medium mt-0.5 truncate">
                {stops[stops.length - 1]?.name}
              </div>
              <div className="text-xs text-gray-400 mt-0.5">Arrived at destination</div>
            </div>
          </div>
        </div>
      </div>
